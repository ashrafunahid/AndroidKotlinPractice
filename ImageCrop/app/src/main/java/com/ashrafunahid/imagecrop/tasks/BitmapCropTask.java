package com.ashrafunahid.imagecrop.tasks;
...
import com.ashrafunahid.imagecrop.models.CropParameters;
import com.ashrafunahid.imagecrop.models.ExifInfo;
import com.ashrafunahid.imagecrop.models.ImageState;
import com.ashrafunahid.imagecrop.utils.FileUtils;
import com.ashrafunahid.imagecrop.utils.ImageHeaderParser;
...
//            crop(resizeScale);
    ...
    private boolean cropImage(float resizeScale) throws IOException {
...
//            Load Original Bitmap
            ...
            Bitmap srcBitmap = BitmapFactory.decodeFile(mImageInputPath, options);

            if (srcBitmap == null) return false;

            // Setup Matrix for rotation and matrix
            Matrix matrix = new Matrix();
            matrix.postRotate(mCurrentAngle);
            // if resizeScale > 1 then scale here

            // Crop the bitmap
            // Crete new bitmap with certain portion from source image
            Bitmap croppedBitmap = Bitmap.createBitmap(srcBitmap,
                    Math.max(0, cropOffsetX), Math.max(0, cropOffsetY),
                    Math.min(mCroppedImageWidth, srcBitmap.getWidth() - Math.max(0, cropOffsetX)),
                    Math.min(mCroppedImageHeight, srcBitmap.getHeight() - Math.max(0, cropOffsetY)),
                    matrix, true);

            // Save the file
            File outputFile = new File(mImageOutputPath);
            try (FileOutputStream out = new FileOutputStream(outputFile)) {
                boolean success = croppedBitmap.compress(mCompressFormat, mCompressQuality, out);

                if (success && mCompressFormat == Bitmap.CompressFormat.JPEG) {
